...
export function HomePage() {
    ...
    const actions = currentUser
        ...
        : [
            ...
            {
                id: 'login', icon: 'user', label: '로그인', variant: 'primary',
    ...
    function handleAddressSubmit(event) {
        ...
        if (!firstMatch?.congressmanId) { setAddressSearchError('해당 주소의 국회의원 정보를 찾지 못했습니다.'); return }
    ...
    function handleNameSubmit(event) {
        ...
        if (!firstMatch?.congressmanId) { setNameSearchError('해당 이름의 국회의원을 찾지 못했습니다.'); return }
        navigateToCongressman(firstMatch.congressmanId)
    }
                    ...
                    {mode === 'address' ? (
                        <form className="search-form" onSubmit={handleAddressSubmit}>
                            ...
                            <div className="input-shell">
                                ...
                                <input id="home-address" className="input-shell__input" type="text"
                                       placeholder="예) 잠실동, 성산동, 송도5동" value={addressQuery}
                                       onChange={(event) => { setAddressQuery(event.target.value); setAddressSearchError('') }} />
                                {addressQuery ? (
                            ...
                            {addressFeedback ? (
                                <p className={`search-form__feedback ${addressFeedback.tone === 'error' ? 'is-error' : ''}`}>{addressFeedback.text}</p>
                            ) : null}
                            <button className="button button--soft" onClick={handleCurrentLocation} type="button">
                                <Icon className="button__icon" name="spark" /><span>현재 위치 자동 감지</span>
                            </button>
                        ...
                        <form className="search-form" onSubmit={handleNameSubmit}>
                            ...
                            <div className="input-shell">
                                ...
                                <input id="home-name" className="input-shell__input" type="text"
                                       placeholder="예) 김, 박준혁" value={nameQuery}
                                       onChange={(event) => { setNameQuery(event.target.value); setNameSearchError('') }} />
                                {nameQuery ? (
                            ...
                            {nameFeedback ? (
                                <p className={`search-form__feedback ${nameFeedback.tone === 'error' ? 'is-error' : ''}`}>{nameFeedback.text}</p>
                            ) : null}
                            {!nameQuery.trim() ? (
                                <div className="quick-chip-row">
                                    {nameQuickFilters.map((filter) => (
                                        <button key={filter.label} className="quick-chip" onClick={() => setNameQuery(filter.query)} type="button">
                                            {filter.label}
                                        </button>
                                    ))}
                                </div>
